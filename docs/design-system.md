...
**Für dieses Projekt ist die Designsprache festgelegt: `data-design="ha"`** (Home Assistant,
Akzent `#18BCF2`). Sie steht fest in `frontend/index.html` am `<html>`-Element und wird nicht zur
Laufzeit umgeschaltet. Begründung: Energy Pilot ist ein Home-Assistant-Addon und läuft im
Ingress-Panel direkt neben der HA-Oberfläche — eine fremde Akzentfarbe würde dort wie ein
Fremdkörper wirken. Die Frage nach der Designsprache (eiserne Regel 10) ist damit für dieses
Projekt beantwortet und wird nicht erneut gestellt.
