...
## D-050 · Stabile, vollständige KI-Vorschlagsfelder (Determinismus + erzwungenes Schema + Vollständigkeits-Fallback)
**Entscheidung:** Die KI muss bei **jedem** Lauf und **jedem** Modell **IMMER alle** je Gerät geforderten Vorschlagsfelder liefern. Bisher schwankte die Ausgabe (mal nur `prio_vorschlag`, mal alles, die Max-Wassertemperatur des Heizstabs fehlte gelegentlich), weil das Gemini-Antwort-Schema **alle** Geräte-Felder optional ließ (`required: ["name"]`, Felder als Vereinigung über alle Geräte) und **kein** Determinismus gesetzt war (Flash-Default `temperature≈1`). Fix als **Defense-in-Depth (4 Schichten)**, alle über die Addon-Config steuerbar: **(A) Schema erzwingt Felder** — `devices` wird ein **OBJECT** (ein Property je Gerätename) statt eines Arrays; jedes Gerät trägt ein eigenes `required` = exakt sein Schreibvertrag (`suggestion_keys`), plus `propertyOrdering`. Nur so lässt sich pro Gerät (unterschiedliche Klassen/Zusatzfelder, Batterie ohne Prio) ein eigenes Pflichtfeld-Set erzwingen; ein gemeinsames Array-`items` kann das nicht. **(B) Determinismus** — `temperature` (Default 0) + fixer `seed` (Default 42) in der `generationConfig` ⇒ gleicher Kontext liefert stabil dieselben Felder. **(C) Validator-Vollständigkeit (provider-agnostisch)** — fehlt ein bekanntes Gerät ganz, wird es ergänzt; fehlt ein Pflichtfeld, füllt EP es **deterministisch aus dem Ist-Zustand** (`freigabe` ← aktuelle technische Freigabe, geschützte Mindestleistung ← technische Mindestleistung/0, Zusatzfeld ← aktueller Lesewert bzw. Typ-Default; fehlende `prio` wird ans Ende der 10er-Rangfolge gereiht). Jede Füllung wird wie eine Klemmung protokolliert. **(D) Gezielte Nachforderung** — vor der deterministischen Füllung fordert EP fehlende Pflichtfelder **einmalig** gezielt nach (`ai_repair_missing`, Default an); scheitert der Aufruf, greift Schicht C (Iron Rule 8, EP blockiert nie).
**Detail:** `gemini_provider.GeminiProvider(temperature, seed)` schreibt beide in `generationConfig` (nur wenn gesetzt). `config.DEFAULTS`/`config.yaml`: `ai_temperature` (float 0–2), `ai_seed` (int), `ai_repair_missing` (bool). `plan_context.build_response_schema` baut je Gerät `_device_response_schema` (required = `suggestion_keys` + `name`, `propertyOrdering`); `build_repair_prompt` hängt die Lückenliste an den Basis-Prompt. `validator`: `_fallback_value`/`_fill_missing_fields` (Füllung vor der Grenzprüfung), `missing_suggestion_fields` (öffentlich, für den Repair-Entscheid), Anfügen fehlender Geräte in `validate`, `_normalize_priorities` reiht fehlende Prios ans Ende. `planner.run`: nach der Erstantwort `missing_suggestion_fields` → optional **ein** Nachforder-Aufruf (`build_repair_prompt`), Übernahme nur bei weniger Lücken, Tokens beider Aufrufe summiert; `_device_entries` akzeptiert die neue Objekt- **und** die frühere Array-Form (Provider-/Modellrobustheit). Version `0.0.34`.
**Quelle:** User-Vorgabe (unterschiedliche KI-Modelle bzw. wiederholte Planläufe lieferten unterschiedliche Vorschlagsfelder — mal nur die Prio, mal alles, Max-Wassertemperatur des Heizstabs vergessen; gefordert ist, dass **IMMER alle** geforderten Vorschlagswerte kommen). → [04](04-ki-provider.md), [07](07-planning-engine.md), [08](08-validierung-sicherheit.md)

## D-051 · Geräte-Tab per Dropdown (ein Gerät je Ansicht) + Custom-Prompt („funktion") je Gerät
**Entscheidung:** Der Geräte-Tab zeigt nicht mehr alle Geräte gestapelt, sondern lässt den User oben **ein** Gerät per **Dropdown** wählen; darunter erscheinen genau dessen `ems_*`-Werte, Zusatz-Entitäten (D-047) und – **neu** – eine **user-gepflegte KI-Beschreibung** dieses Geräts. Die Beschreibung ist ein Freitext, der der KI **Funktion/Besonderheiten** des konkreten Geräts erklärt (z.B. „versorgt die Fußbodenheizung, träge, darf bevorzugt mittags laufen"). Sie ist rein **advisorisch**: sie geht als Kontext-Feld `funktion` **je Gerät** in den Planungs-Prompt ein (nur wenn gesetzt → Datenminimum, Iron Rule 7) und wird **nie** ans HEMS übergeben. Damit gibt es zwei Prompt-Ebenen: der **globale** Planungs-Prompt (D-043) beschreibt die Gesamtstrategie, die **geräte-spezifische** `funktion` erklärt einzelne Geräte. Leerer Text löscht die Beschreibung (kein Prompt = Standard).
**Detail:** Neue Tabelle `device_prompts(device_name PK, prompt, updated_at)` (Migration 7); Modul `device_prompts.py` (`load_device_prompts`/`get_device_prompt`/`set_device_prompt`, leer ⇒ Delete). `Device.ai_prompt` + `DeviceConstraint.ai_prompt`; `apply_extras(devices, extras, prompts)` mergt die Prompts nach der HEMS-Discovery (wie die Zusatz-Entitäten, D-047). `plan_context._condense_constraint` gibt `funktion` nur bei gesetztem Text aus; `DEFAULT_PLANNING_PROMPT` bekommt eine Regelzeile dazu. Neuer Endpoint `POST /api/devices/prompt {device_name, prompt}` (→ `reapply_device_extras`, wirkt sofort, kein HEMS-Reload; Audit über `_audit_prompt` mit `subject=device_name`). `/api/devices` liefert `ai_prompt` je Gerät. Frontend (`index.html`): Dropdown `#device-select` (Auswahl bleibt über die 10s-Auto-Aktualisierung erhalten), je Gerät ein Beschreibungs-Editor (Speichern/Leeren); der Editier-Guard `isEditingDevices` schützt Zusatz-Formular **und** Beschreibung vor dem Überschreiben. Version `0.0.35`.
**Quelle:** User-Vorgabe (Geräte-Tab übersichtlicher: Dropdown statt alle Geräte auf einer Seite; zusätzlich pro Gerät ein Custom-Prompt für die KI, z.B. was das Gerät für eine Funktion hat). → [02](02-homeassistant.md), [07](07-planning-engine.md)

## D-052 · Zusatz-Entitäten: „In Original schreiben" (Original-Schreibweg für Helfer)
**Entscheidung:** Zusätzlich zur Checkbox „KI liefert Vorschlagswert" (D-047) gibt es je Zusatz-Entität eine weitere Checkbox **„In Original schreiben"**, die **nur** aktivierbar ist, wenn der KI-Vorschlag selbst aktiv ist (kein Original-Schreibweg ohne Vorschlagswert). Ist sie gesetzt, schreibt EP den KI-Vorschlag **zusätzlich** zum bisherigen `sensor.ep_<obj>_vorschlag` **direkt in die Original-Entität** zurück. Das gilt ausdrücklich nur für echte, schreibbare **Helfer**-Domänen (`input_number`/`number`, `input_boolean`, `input_datetime`, `input_text`/`text`, `input_select`/`select`) — bei `sensor.*`-Quellen (grundsätzlich read-only) bleibt es unverändert bei **nur** dem `_vorschlag`-Sensor; die Checkbox hat dort keine Wirkung. Das durchbricht bewusst den bisherigen V1-Grundsatz „Vorschläge sind rein advisorisch, EP schreibt nie in User-Entitäten" (D-047) — aber **nur** dort, wo der User es je Zusatz-Entität explizit anfordert.
**Detail:** `DeviceExtra.write_original: bool` (neues Feld, Default `False`); `is_writable_helper` (Domäne in fester Helfer-Allowlist `_WRITABLE_HELPER_DOMAINS`, exkl. `sensor`/`switch`/`light`/`binary_sensor`); `should_write_original` = `write_original and ai_suggestion and is_writable_helper` — die einzige Stelle, die den **effektiven** Schreibweg bestimmt (schützt gegen inkonsistente/rohe DB-Kombinationen). Persistenz: `device_extras`-Tabelle **Migration 8** (Spalte `write_original`), `device_extras.py` (`upsert_extra`/`_row_to_extra`/`load_extras` erweitert). Schreibweg: `ha_client.HAClient.call_service()` (neu, `POST /api/services/<domain>/<service>`, **Service-Call statt rohem State-Overwrite** — damit greifen HAs eigene Validierung/Min-Max/Optionspool). `suggestion_publisher.build_original_writes()` leitet je `should_write_original`-Zusatzentität den passenden Service aus `kind` ab (`_service_payload`): `number`→`set_value{value}`, `text`→`set_value{value}`, `select`→`select_option{option}`, `datetime`→`set_datetime{datetime}`, `bool`→`turn_on`/`turn_off`. `publish_suggestions()` ruft diese zusätzlich zum bisherigen `set_state`-Schreibweg auf, Fehler je Entität abgefangen (Iron Rule 8, blockiert nie). Web: `POST /api/devices/extras` validiert `write_original` nur mit `ai_suggestion` (sonst 400); `GET /api/devices` liefert `write_original`/`is_writable_helper`/`should_write_original` je Zusatz-Entität. UI (`index.html`): zweite Checkbox im Zusatz-Entitäten-Formular, per JS an die Vorschlag-Checkbox gekoppelt (disabled + entkoppelt, wenn kein Vorschlag aktiv); Tabellenspalte zeigt den effektiven Zustand inkl. Hinweis, wenn `write_original` gesetzt aber wegen `sensor.*` wirkungslos ist. Version `0.0.36`.
**Quelle:** User-Vorgabe (zusätzliche Checkbox „In Original schreiben", nur wählbar wenn KI-Vorschlag gefordert; bei Aktivierung wird der Vorschlagswert zusätzlich zum `ep_*_vorschlag`-Sensor in die Original-Entität geschrieben, sofern es sich um einen Helfer handelt; bei `sensor.*` entsteht trotzdem nur der Vorschlags-Sensor). → [05](05-daten-und-speicherung.md), [09](09-ui-ingress.md)

## D-053 · One-Call-Wetter an die KI: stündliches Tagesfenster (heute, bis 21 Uhr) + 5-Tage-Ausblick
**Entscheidung:** Bei `weather.source: onecall` bekommt die KI im Planungskontext **zwei** Wetterblöcke statt (wie bisher, D-044) genau **einer** ungefensterten Timeline. **`hourly`:** die **stündliche** Reihe **nur für den heutigen Tag**, vom aktuellen Zeitpunkt (**frühestens ab 6 Uhr**) bis **21 Uhr Ortszeit** — z.B. um 11:30 → 12:00–21:00; vor 6 Uhr → ab 6:00; nach 21 Uhr bleibt die Stundenreihe **leer**. **`daily`:** ein **Tagesausblick der nächsten 5 Tage ab morgen** (der heutige Tag steckt bereits in der Stundenreihe) mit Temperatur inkl. Tages-Min/Max, Bewölkung und Regenwahrscheinlichkeit. Die Ortszeit ergibt sich aus dem OWM-`timezone_offset` der Wetter-Zone. Damit ist die geparkte Frage **O4** („mehrere Timelines gleichzeitig ans LLM", claude-fragen-v10) umgesetzt. UI-Anzeige (roher Timeline-Snapshot) und der Nicht-Weg ans HEMS bleiben unverändert; `forecast3h` bleibt wie gehabt (kompakt/voll, D-043).
**Detail:** `plan_context.py`: neues `_condense_onecall(weather, *, now)` liefert `{source, units, hourly:{resolution,hinweis,slots}, daily:{hinweis,slots}}`; Fenster-Helfer `_hourly_window_slots` (Grenzen `HOURLY_WINDOW_START_HOUR=6`/`HOURLY_WINDOW_END_HOUR=21` als echte Ortszeit-Zeitpunkte) und `_daily_next_days_slots` (`DAILY_FORECAST_DAYS=5`, ab morgen). `weather.onecall.llm_timeline` wählt jetzt die **Auflösung der Stundenreihe** (`15min`/`1h`, sonst Fallback `1h`); die Tagesreihe kommt fest aus `1day`. `build_context(..., now=…)` und `_condense_weather(..., now=…)` reichen den Zeitpunkt durch (Default = jetzt, UTC); `planner.run` übergibt sein `now`. `OneCallCollector.snapshot()` trägt je Timeline zusätzlich `timezone_offset_s` (für die UTC→Ortszeit-Umrechnung). Version `0.0.47`.
**Quelle:** User-Vorgabe ([../upcoming_changes.md](../upcoming_changes.md)). → [06](06-prognosen.md), [07](07-planning-engine.md)

## D-054 · One-Call-Wetter: freie Kombination der Vorhersagemodelle ans LLM (Schalter statt Einzel-Select)
**Entscheidung:** Der User muss sich nicht mehr für **ein** Vorhersagemodell entscheiden, das an die KI geht (bisher `weather.onecall.llm_timeline`, ein Single-Select, D-044/D-053). Stattdessen wählt er per **Schalter** (`enable_15min`/`enable_1h`/`enable_1day`) eine **beliebige Kombination**: **jedes aktivierte Modell wird abgerufen UND fließt gemeinsam in den KI-Kontext**. Damit ist das Abruf- und das KI-Kontext-Schalten **vereint** — kein separates LLM-Auswahlfeld mehr. Die Fensterregeln aus D-053 bleiben je Modell erhalten: intraday-Modelle (15min/1h) auf **heute** (ab jetzt, frühestens 6 Uhr, bis 21 Uhr Ortszeit), das Tagesmodell (1day) auf die **nächsten 5 Tage ab morgen**.
**Detail:** `weather.py`: `OneCallConfig.llm_timeline` + `DEFAULT_LLM_TIMELINE` entfernt (inkl. Parsing). `plan_context._condense_onecall(weather, *, now)` iteriert jetzt über **alle** aktiven Timelines und liefert `weather.models[<res>] = {aufloesung, zeitraum, slots}` (nur Modelle mit `enabled` **und** vorhandenen Slots; gate schützt gegen deaktivierte, aber noch gecachte Reihen). `DEFAULT_PLANNING_PROMPT` beschreibt `weather.models`. `OneCallCollector.snapshot()` liefert statt `llm_timeline` nun `ai_models` (= aktive Modelle). `config.yaml`/`config.py`: `llm_timeline` aus Optionen **und** Schema entfernt (verbleibt ein alter Wert in `options.json`, wird ignoriert). UI (`app.js`): „KI nutzt: <aktive Modelle>" statt „KI-Timeline". Version `0.0.48`.
**Quelle:** User-Vorgabe („Checkboxen auf Schalter umbauen, damit man eine Kombination statt eines einzelnen Vorhersagemodells wählen kann"). → [06](06-prognosen.md), [09](09-ui-ingress.md)
...
- **v9-O2/O3 erledigt → D-045** (Pagination 1–5 + Pflicht-Tages-Call-Budget; Unwetter-Alerts aufgenommen); **v10-O4 erledigt → D-053** (Stundenreihe heute bis 21 Uhr + 5-Tage-Ausblick ans LLM). **Geparkt für Zukunft/V2+:** **O1** `current`-Ist-Wetter, **O5** historische Daten — fortgeführt in [../claude-fragen/claude-fragen-v10.md](../claude-fragen/claude-fragen-v10.md). **(v8-W1/W2/W3 → D-043/D-044; W4–W6 bestätigt.)**
