...
- **Wetter (D-042):** **direkt im EP** über die OpenWeatherMap-„5 day / 3 hour forecast"-API abgerufen (eigene externe Quelle, **nicht** über HA-Sensoren wie die PV-Prognose).
  ...
  - **Quelle umschaltbar (D-044):** `weather.source` = `forecast3h` (5-Tage/3-Stunden, **Default**) **oder** `onecall` (**One Call API 4.0**, Abo „One Call by Call" nötig). Bei `onecall` getrennte Timelines **15min/1h/1day** über `onecall_client.py`/`OneCallCollector`, je einzeln per **Schalter** aktivierbar mit **eigenem Refresh-Intervall** (`weather.onecall.enable_*`/`refresh_*`). **KI-Kontext (D-053/D-054):** **jedes aktivierte Modell** fließt gemeinsam in den Kontext (`weather.models[<res>]`) — **freie Kombination**, kein Einzel-Select mehr (`llm_timeline` entfallen). Fensterung je Modell: intraday (`15min`/`1h`) **nur für heute** (ab jetzt, frühestens 6 Uhr, bis 21 Uhr Ortszeit), das Tagesmodell (`1day`) für die **nächsten 5 Tage ab morgen**.
  ...
  - **V1-Scope:** **Noch nicht** als HA-Sensor und **nicht** über die HEMS-API (eigener späterer Schritt). **O4 (mehrere Timelines ans LLM) umgesetzt** — Stundenreihe heute + 5-Tage-Ausblick (D-053). Verbleibende One-Call-Punkte (current-Ist-Wetter, Historie) → claude-fragen-v10 (geparkt O1/O5).
